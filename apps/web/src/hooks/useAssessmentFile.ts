...
export function useAssessmentFile() {
    const handleSubmitAssessment = (
        ...
        const assessmentData = {
            ...
            leveling: categories.reduce((acc, category) => {
                ...
                acc[category.title] = {
                    level: median,
                    notes: category.competencies.reduce(
                        (noteAcc, competence) => {
                            const level = categorySelections[competence.name];
                            if (level !== undefined) {
                                const levelFeedback =
                                    categoryFeedback[competence.name]?.[level];
                                noteAcc[competence.name] = {
                                    level: `L${level}`,
                                    evidence: levelFeedback?.evidence || "",
                                    advice:
                                        levelFeedback?.nextLevelFeedback || "",
                                };
                            }
                            return noteAcc;
                        },
        ...
        link.href = url;
        link.download = `${teamMemberName.replace(
            /\s+/g,
            "_"
        )}_assessment.json`;
    ...
    const handleOpenAssessment = (
        ...
        setFeedback: (
            feedback: Record<
                ...
                Record<
                    ...
                    Record<
                        string,
                        { evidence: string; nextLevelFeedback: string }
                    >
                >
            >
        ) => void,
        ...
        try {
            ...
            Object.entries(data.leveling).forEach(
                ([categoryTitle, categoryData]: [string, any]) => {
                    console.log(
                        "Processing category:",
                        categoryTitle,
                        categoryData
                    ...
                    if (categoryData.notes) {
                        Object.entries(categoryData.notes).forEach(
                            ([competence, noteData]: [string, any]) => {
                                console.log(
                                ...
                                if (typeof noteData.level === "number") {
                                    ...
                                    const levelStr = noteData.level
                                        .toString()
                                        .replace(/^L/i, "");
                                    level = parseInt(levelStr, 10);
                                } else {
                                    console.warn(
                                        "Invalid level format for",
                                        competence,
                                        ":",
                                        noteData.level
                                    );
                                ...
                                newSelections[categoryTitle][competence] =
                                    level;
