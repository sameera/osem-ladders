...
export function createTeamApi(api: UseApiReturn) {
  return {
    ...
    async fetchTeams(query?: ListTeamsQuery): Promise<ListTeamsResponse> {
      ...
      const url = `/growth/admin/teams${queryString ? `?${queryString}` : ''}`;
    ...
    async fetchTeam(teamId: string): Promise<Team> {
      const response = await api.get<ApiResponse<Team>>(`/growth/admin/teams/${teamId}`);
      if (!response.success || !response.data) {
    ...
    async createTeam(request: CreateTeamRequest): Promise<Team> {
      const response = await api.post<ApiResponse<Team>>('/growth/admin/teams', request);
      if (!response.success || !response.data) {
     ...
     */
    async updateTeamManager(teamId: string, managerId: string | null): Promise<Team> {
      const response = await api.patch<ApiResponse<Team>>(
        `/growth/admin/teams/${teamId}/manager`,
        { managerId } as AssignManagerRequest
      );
      if (!response.success || !response.data) {
        throw new Error(response.error?.message || 'Failed to update team manager');
      }
      return response.data;
    },
  };
