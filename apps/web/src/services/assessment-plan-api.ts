...
export function createAssessmentPlanApi(api: UseApiReturn) {
  return {
    ...
    async fetchPlans(
      ...
      const url = `/growth/teams/${teamId}/plans${queryString ? `?${queryString}` : ''}`;
     ...
     */
    async fetchPlan(teamId: string, season: string): Promise<AssessmentPlan> {
      const url = `/growth/teams/${teamId}/plan/${encodeURIComponent(season)}`;
    ...
    async createOrUpdatePlan(
      ...
      const response = await api.put<ApiResponse<CreatePlanResponse>>(
        `/growth/teams/${teamId}/plans`,
        input
      );
      if (!response.success || !response.data) {
        throw new Error(response.error?.message || 'Failed to create/update plan');
      }
      return response.data;
    },
  };
